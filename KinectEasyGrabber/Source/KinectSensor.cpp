...
HRESULT KinectSensor::Init(NUI_IMAGE_TYPE depthType, NUI_IMAGE_RESOLUTION depthRes, BOOL bNearMode, BOOL bFallbackToDefault, NUI_IMAGE_TYPE colorType, NUI_IMAGE_RESOLUTION colorRes, BOOL bSeatedSkeletonMode)
{
	...
	m_colorCoordinates = new LONG[width*height*2];
...
/*
	...
	m_KinectSensorPresent = SUCCEEDED(hr);
...
HRESULT KinectSensor::GetCameraConfig(FT_CAMERA_CONFIG* cameraConfig)
{
    return m_KinectSensorPresent ? GetVideoConfiguration(cameraConfig) : E_FAIL;
}
...
void KinectSensor::Release()
{
    ...
    if (m_bNuiInitialized && m_pNuiSensor)
    {
...
void KinectSensor::GotDepthAlert( )
{
	...
	DWORD width = m_DepthBuffer->GetWidth();
	DWORD height = m_DepthBuffer->GetHeight();

	NUI_IMAGE_RESOLUTION cColorResolution = SizeToImageResolution(width, height);
	NUI_IMAGE_RESOLUTION cDepthResolution = SizeToImageResolution(width, height);

    hr = m_pNuiSensor->NuiImageGetColorPixelCoordinateFrameFromDepthPixelFrameAtResolution(
		  cColorResolution,
		  cDepthResolution,
		  width*height,
		  (USHORT*)m_DepthBuffer->GetBuffer(),
		  width*height*2,
 		  m_colorCoordinates
         );
}

NUI_IMAGE_RESOLUTION KinectSensor::SizeToImageResolution(DWORD width, DWORD height)
{
    NUI_IMAGE_RESOLUTION image_resolution = NUI_IMAGE_RESOLUTION_INVALID;

    if(width == 80 && height == 60)
    {
        image_resolution = NUI_IMAGE_RESOLUTION_80x60;
    }
    else if(width == 320 && height == 240)
    {
        image_resolution = NUI_IMAGE_RESOLUTION_320x240;
    }
    else if(width == 640 && height == 480)
    {
        image_resolution = NUI_IMAGE_RESOLUTION_640x480;
    }
	else if(width == 1280 && height == 960)
    {
		image_resolution = NUI_IMAGE_RESOLUTION_1280x960;
    }
