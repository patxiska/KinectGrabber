...
public:
	...
	//HRESULT Init(NUI_IMAGE_TYPE depthType, NUI_IMAGE_RESOLUTION depthRes, BOOL bNearMode, BOOL bFallbackToDefault, NUI_IMAGE_TYPE colorType, NUI_IMAGE_RESOLUTION colorRes);
    void Release();
	...
	LONG*			GetColorCoordinates() { return(m_colorCoordinates); };
    LARGE_INTEGER   GetDepthTimeStamp(){ return(m_depthTimeStamp); };
    LARGE_INTEGER   GetColorTimeStamp(){ return(m_colorTimeStamp); };
	int				GetFramesCount() {return m_FramesTotal;}
...
private:
	...
	static NUI_IMAGE_RESOLUTION SizeToImageResolution(DWORD width, DWORD height);
};
