...
protected:
	...
	BOOL						ShowRecordingInfo(HDC hdc, int width, int height);
...
BOOL MultiFace::InitInstance(HINSTANCE hInstance, PWSTR lpCmdLine, int nCmdShow)
{
    ...
    swprintf_s(szTitleComplete, L"%s -- Depth:%s:%s Color:%s:%s NearMode:%s", szTitle,
    ...
    return SUCCEEDED(m_FTHelper.Init(m_hWnd, m_nbUsers, NULL /*FTHelperCallingBack*/, this, NULL, this, m_depthType, m_depthRes, m_bNearMode, m_colorType, m_colorRes, m_bSeatedSkeletonMode));
	//return SUCCEEDED(m_KinectSensor.Init(m_depthType, m_depthRes, m_bNearMode, FALSE, m_colorType, m_colorRes, m_bSeatedSkeletonMode));
}
...
BOOL MultiFace::ShowRecordingInfo(HDC hdc, int width, int height)
{
	UINT frame = m_FTHelper.GetFramesCount();
	UINT max_frames = m_FTHelper.GetMaxFrames();

	char time_text[KINECT_MAX_PATH];
	char recStatus[KINECT_MAX_PATH];

	sprintf(time_text,"%.2f of %.2f",float(frame)/33,float(max_frames)/33);

	KINECT_RECORDER_STATUS stat = m_FTHelper.GetRecordingStatus();
	switch (stat){
		case KINECT_RECORDER_STATUS_DUMPED:
			strcpy(recStatus,"DUMPED");
			break;
		case KINECT_RECORDER_STATUS_RECORDING:
			strcpy(recStatus,"RECORDING ");
			strcat(recStatus,time_text);
			break;
		case KINECT_RECORDER_STATUS_SAVING:
			strcpy(recStatus,"SAVING");
			break;
		case KINECT_RECORDER_STATUS_STOPPED:
			strcpy(recStatus,"STOPPED");
			break;
		default:
			strcpy(recStatus,"UNDEFINED");
			break;
	}

	TCHAR ttext[KINECT_MAX_PATH];
	mbstowcs (ttext, recStatus, KINECT_MAX_PATH);
	UINT x = 0; UINT y = 0;

	TextOut(hdc,x,y,ttext,strlen(recStatus));
...
BOOL MultiFace::ShowVideo(HDC hdc, int width, int height, int originX, int originY)
{
	...
	//IFTImage* colorImage = m_KinectSensor.GetVideoBuffer();
    if (colorImage)
    {
...
BOOL MultiFace::PaintWindow(HDC hdc, HWND hWnd)
{
 ...
 //   errCount += !ShowVideo(hdc, width - halfWidth, height, halfWidth, 0);
	//ShowRecordingInfo(hdc, width - halfWidth, height);

    ShowVideo(hdc, width, height, 0, 0);
	ShowRecordingInfo(hdc, width, height);
...
void MultiFace::ParseCmdString(PWSTR lpCmdLine)
{
	...
	const WCHAR KEY_FRAMES_MODE[]							= L"-Frames";
	const WCHAR KEY_TIME_MODE[]								= L"-Time";
    ...
    for(int i = 0; i < argc; i++)
    {
        ...
        else if(0 == wcsncmp(token, KEY_FRAMES_MODE, ARRAYSIZE(KEY_FRAMES_MODE)))
        {
            tokenType = TOKEN_TOTAL_FRAMES;
        }
        else if(0 == wcsncmp(token, KEY_TIME_MODE, ARRAYSIZE(KEY_TIME_MODE)))
        {
            tokenType = TOKEN_TOTAL_TIME;
        }
		...
		else if(tokenType == TOKEN_TOTAL_FRAMES)
		{
            UINT frames = _wtoi(context);
			m_FTHelper.SetMaxFrames(frames);
		}
		else if(tokenType == TOKEN_TOTAL_TIME)
		{          
			UINT frames = _wtof(context)*33;
			m_FTHelper.SetMaxFrames(frames);
		}
    }
