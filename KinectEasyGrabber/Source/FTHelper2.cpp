...
{
	if( m_isRecording && m_KinectSensorPresent &&
	    m_KinectSensor.GetFramesCount() > 0 && m_KinectSensor.GetVideoBuffer() && m_KinectSensor.GetDepthBuffer() )
	{
			m_KinectRecorder.addRGBXFrame(reinterpret_cast<BYTE*>(m_KinectSensor.GetVideoBuffer()->GetBuffer()), m_KinectSensor.GetColorTimeStamp() );
			m_KinectRecorder.addDepthFrame(reinterpret_cast<USHORT*>(m_KinectSensor.GetDepthBuffer()->GetBuffer()), m_KinectSensor.GetDepthTimeStamp(), m_KinectSensor.GetColorCoordinates());
			//m_KinectRecorder.addColorCoordinatesFrame(m_KinectSensor.GetColorCoordinates());
	}
	//else
	//{
	//    HRESULT hrFT = E_FAIL;
	...
	//		{
	//			hrCopy = m_KinectSensor.GetDepthBuffer()->CopyTo(m_depxthImage, NULL, 0, 0);
	//		}
	//		// Do face tracking
	//		if (SUCCEEDED(hrCopy))
	//		{
...
DWORD WINAPI FTHelper2::FaceTrackingThread()
{
	...
	hr = m_KinectRecorder.Init(m_depthType, m_depthRes, m_bNearMode, m_colorType, m_colorRes, m_bSeatedSkeleton,"C:\\data","seq1");
	...
	{
        MessageBoxW(m_hWnd, L"Could not initialize the Kinect recorder.\n", L"Face Tracker Initialization Error\n", MB_OK);
        return 1;	
	}
